...
export default function LevelEdit({ level, companies }: Props) {
  const { url } = usePage();
  const [formData, setFormData] = useState({
    name: level.name,
    description: level.description || '',
    level_order: level.level_order,
    company_id: level.company_id.toString(),
    status: level.status,
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    router.put(route('organization.level.update', level.id), formData, {
      onError: (errors) => setErrors(errors),
    });
  };

  return (
    <AppLayout title="Edit Level" breadcrumbs={breadcrumbs}>
      <Card>
        <CardHeader>
          <CardTitle>Edit Level: {level.name}</CardTitle>
        </CardHeader>
        <CardContent>
