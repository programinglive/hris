...
>
	...
	<div class="flex justify-between" x-show="!open" >
		...
		>
			...
			<a
				href="{{ asset('leave_type_data.xlsx') }}"
				class="
			        px-4 py-2
			        rounded-md
			        text-blue-500
			...
			@enderror
			<button
				wire:click="importAttendanceOvertime"
				type="button"
	...
	<div x-show="open">
		<livewire:attendance-overtime-form />
	</div>
				...
				>
					...
					@forelse($leaveTypes as $leaveType)
						<tr>
							...
							>
								{{ $leaveType->company_name }}
							</td>
							<td
								class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-500"
							>
								{{ $leaveType->branch_name }}
							</td>
							<td
								class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-500"
							>
								{{ $leaveType->employee_nik }}
							</td>
							<td
								class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-500"
							>
								{{ $leaveType->employee_name }}
							</td>
							<td
								class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-500"
							>
							...
							>
								<button
									wire:click="$dispatch('edit', { nik: '{{ $leaveType->nik }}'})"
									class="text-gray-500 hover:text-sky-700 text-end"
								>
									Edit
								</button>
								<button
									wire:click="$dispatch('delete', { nik: '{{ $leaveType->nik }}'})"
									class="text-gray-500 hover:text-sky-700 text-end"
								>
									Delete
								</button>
