					...
					@forelse($permissions as $permission)
						<tr>
							...
							<td class="px-6 py-4 whitespace-nowrap text-end text-sm font-medium flex flex-col" style="width: 100px">
								<button class="text-gray-500 hover:text-sky-700 text-end" wire:click="$dispatch('edit', { name: '{{ $permission->name }}'})">Edit</button>
								<button class="text-gray-500 hover:text-sky-700 text-end" wire:click="$dispatch('delete', { name: '{{ $permission->name }}'})">Delete</button>
							</td>
						</tr>
					@empty
						<tr>
