						...
						<tr>
							<td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-500">
								<div class="flex justify-between">
									<div>Department</div>
									<div>{{ $subDivision->department->name }}</div>
								</div>
								<div class="flex justify-between">
									<div>Code</div>
									<div>{{ $subDivision->code }}</div>
								</div>
								<div class="flex justify-between">
									<div>Name</div>
							...
							<td class="px-6 py-4 whitespace-nowrap text-end text-sm font-medium flex flex-col" style="width: 100px">
								<button class="text-gray-500 hover:text-sky-700 text-end" wire:click="$dispatch('edit', { code: '{{ $subDivision->code }}'})">Edit</button>
								<button class="text-gray-500 hover:text-sky-700 text-end" wire:click="$dispatch('delete', { code: '{{ $subDivision->code }}'})">Delete</button>
							</td>
						</tr>
					@empty
						<tr>
